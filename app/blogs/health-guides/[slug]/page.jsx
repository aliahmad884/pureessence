...
export default function BlogSlug({ params }) {
    ...
    let html = ``;
    useEffect(() => {
        setPostUrl(`${window.location.origin}/blogs/health-guides/${slug}`)
        const clearHtml = DOMPurify.sanitize(html)
        setSanitizedHtml(clearHtml)
        console.log(encodeURIComponent(`${window.location.origin}/blogs/health-guides/${slug}`))
    }, [])
    return (
        <>
                ...
                <div className="readerCont">
                    <div className="readerHeader">
                        <div className="breadCrumbs" aria-label="breadcrumb">
                            <Link href={'/blogs/health-guides'}><FontAwesomeIcon icon={faArrowLeft} />  Health Guides</Link>
                        </div>
                        <div className="postHeader">
                            <p>Published October 9, 2024 in <Link href={'/blogs/health-guides'}>Guides</Link></p>
